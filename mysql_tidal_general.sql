...
SELECT
    artist_name
  , COUNT(artist_name) AS artist_play_count
FROM tidal.streaming_working
...
SELECT
    track_title
  , COUNT(track_title) AS song_play_count
FROM tidal.streaming_working
...
SELECT
    RANK() OVER(PARTITION BY country_name ORDER BY COUNT(track_title) DESC) AS ranking
  , country_name
  , artist_name
  , track_title
  , COUNT(track_title) AS play_count
...
WITH cte AS (
  SELECT
    ...
    , track_title
    , stream_duration_ms
    , LEFT(CAST(entry_date AS STRING), 4) AS yr
  FROM tidal.streaming_working
)
SELECT
...
WITH cte AS (
  SELECT
    ...
    , track_title
    , stream_duration_ms
    , LEFT(CAST(entry_date AS string), 7) AS year_month
  FROM tidal.streaming_working
)
SELECT
    ROUND(SUM(cte.stream_duration_ms)) AS streaming_total_ms
...
SELECT
    LEFT(CAST(entry_date AS string), 10) AS yr_mo_d
  , COUNT(track_title) AS play_count
FROM tidal.streaming_working
GROUP BY yr_mo_d
;
...
SELECT
    artist_name
  , track_title
  , LEFT(CAST(entry_date AS string), 10) AS play_date
FROM tidal.streaming_working
...
SELECT
    LEFT(CAST(entry_date AS string), 10) AS y_mo_d
  , *
FROM tidal.streaming_working
